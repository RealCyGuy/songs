...
var filtered = computed(() => {
  return data.items.filter((song) => {
    return (
      song.title.toLowerCase().includes(search.value) ||
      song.channel.toLowerCase().includes(search.value)
    );
  });
});
...
<template>
  <div
    class="flex flex-col items-center px-3 sm:px-10 max-w-2xl w-full mx-auto"
  >
    <div class="flex justify-between w-full mb-5">
      ...
      />
      <a
        class="py-2 px-5 bg-[#6600E8] rounded-lg hover:bg-white hover:text-black hover:rounded-none duration-500"
        href="https://gist.github.com/RealCyGuy/43291c70392ca6ee335a6871175a54c8"
        >Download</a
      >
    </div>
    <div class="w-full p-3">
      <p class="text-[#c7ddf5] w-full">
        <span class="text-white">{{ data.items.length }}</span> songs (<span
          class="text-white"
      ...
      <p class="text-[#c7ddf5] w-full">
        <span class="text-white">{{ hours }}</span> hours,
        <span class="text-white">{{ minutes }}</span> minutes,
        <span class="text-white">{{ seconds }}</span> seconds total time
      </p>
      <p class="text-[#c7ddf5] w-full">
        or
        <span class="text-white">{{ (duration / 864.0).toFixed(4) }}%</span> of
        a day,
      </p>
      <p class="text-[#c7ddf5] w-full">
        which is
        <span class="text-white">{{ (duration / 6048.0).toFixed(4) }}%</span> of
        a week,
      </p>
      <p class="text-[#c7ddf5] w-full">
        and
        <span class="text-white">{{ (duration / 315569.52).toFixed(4) }}%</span>
        of a year!
      </p>
    </div>
    <div class="w-full mb-5">
      ...
      <div
        class="inline-block bg-purple-800 from-[#7147b4] to-[#2a2cc4] bg-gradient-to-r rounded-md"
      >
        <div
